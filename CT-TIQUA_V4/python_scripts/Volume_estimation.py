...
def ventricle_volume_computation(seg_path, atlas_path, out_csv_path):

    #prendre en compte les labels de l'atlas registered: 
    # Basal_cisterns: 43
    # Third_ventricle: 1
    # Fourth_ventricle: 2
    ...
    atlas_region_ids = [1, 2, 3, 4, 43]

    ventricle_seg = nib.load(seg_path)
    atlas = nib.load(atlas_path)

    if ventricle_seg.shape != atlas.shape:
        seg_h = nibabel.processing.conform(from_img=seg_h,out_shape=atlas.shape, voxel_size=atlas.header.get("pixdim")[1:4], order = 0)

    ventricle_seg_data = ventricle_seg.get_fdata()
    atlas_data = atlas.get_fdata()


    ventricle_volumes = [0, 0, 0, 0, 0]

    for i in ventricle_seg_data.shape[0]:
        for j in ventricle_seg_data.shape[1]:
            for k in ventricle_seg_data.shape[2]:
                if ventricle_seg_data[i,j,k] != 0 and int(atlas_data[i,j,k]) in atlas_region_ids:
                    ventricle_volumes[atlas_data[i,j,k]] += 1

    with open(out_csv_path, 'w', newline='') as csvfile:
        wr = csv.writer(csvfile, quoting=csv.QUOTE_ALL)
        wr.writerow(["Third_ventricle", "Fourth_ventricle", "Right_lateral_ventricle", "Left_lateral_ventricle", "Basal_cisterns"])
        wr.writerow(ventricle_volumes)